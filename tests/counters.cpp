...
int main(int argc, char** argv) {
    ...
    const double measured = ppe::probe::measure_clock_ghz(0.02);
    if (sup.available) {
        // A plausible range rather than a value: this is the runner's clock.
        expect_true("measured clock is plausible (0.1-10 GHz)",
                    measured > 0.1 && measured < 10.0);
    } else {
        expect_true("measurement returns 0 when denied, never a guess",
                    measured == 0.0);
    }

    std::printf("\nbest_clock() falls back rather than reporting nothing:\n");
    const ppe::clock_reading r = ppe::best_clock(0.02);
    expect_true("measured flag matches counter availability",
